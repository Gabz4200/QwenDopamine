"""Kaggle 2xT4 GPU smoke-test + FineWeb-Edu Micro training for Pure Recurrent Multi-Head GatedSurpriseNetAdam (1.3B).
...
Architecture:
  ...
  SwiGLU MLP (intermediate=5504), and GatedSurpriseNetAdam token mixer across ALL 24 layers
...
Part 1 — Sanity checks & 1.3B Architecture Inspection
  ...
  2. Synthetic overfit on multi-head pure recurrent GPT model (all layers GatedSurpriseNetAdam).
...
if has_cuda:
    ...
    if num_gpus > 1 and WORLD_SIZE == 1:
        _script_name = (
            ...
            else "notebooks/test_gated_surprise_net_gpu.py"
        )
        print(
...
def inspect_1b_architecture() -> None:
    ...
    print("=" * 60)
    print(f"1.3B Multi-Head Pure Recurrent Model Specification ({cfg_1b.name} - GatedSurpriseNetAdam)")
    print("=" * 60)
    print(
    ...
    print(
        f"  Recurrent Layers:        {stats['num_surprise_layers']}x GatedSurpriseNetAdam (All 24 layers)"
    )
    print(
...
def run_synthetic_overfit() -> tuple[float, float, bool]:
    """Verify learning on a small multi-head pure recurrent GPT model (all layers GatedSurpriseNetAdam)."""
    torch.manual_seed(0)
    vocab_size = 64
    seq_len = 32
    batch_size = 4
    ...
    if IS_MAIN:
        ...
        print(f"[synth] Multi-head: {cfg.n_head} heads | All {cfg.n_layer} layers use GatedSurpriseNetAdam (0 self-attention)")
...
def run_serial_chunk_parity() -> bool:
    ...
    u = torch.rand(bs, t, h, d, device=device, dtype=dtype)

    memory = SurpriseMemory(num_heads=h, head_k_dim=d, head_v_dim=d).to(
        device=device, dtype=dtype
    )

    out_s, _, nll_s = memory.serial_scan(q, k, v, g, b, w, u)
    out_c, _, nll_c = memory.chunk_parallel_training_scan(
        q, k, v, g, b, w, u, chunk_size=16
    )
...
def plot_metrics(
    ...
    fig.suptitle(
        "Multi-Head Pure Recurrent GatedSurpriseNetAdam (1.3B) — FineWeb-Edu Micro Training Metrics",
        fontsize=13,
    )
...
def main() -> None:
    ...
    if IS_MAIN:
        print(
            ...
            f"layers={lm_cfg.n_layer} (16 heads, 100% GatedSurpriseNetAdam, 0 self-attention)"
        )
    ...
    if IS_MAIN:
        ...
        print(
            f"  Architecture:            Multi-Head Pure Recurrent (1.3B, Layers: {lm_cfg.n_layer}, Heads: {lm_cfg.n_head}, Mixer: GatedSurpriseNetAdam, 0 Self-Attention)"
        )
        print(f"  Train steps:             {train_cfg.num_steps}")
        print("  Result: Multi-Head Pure Recurrent GatedSurpriseNetAdam 1.3B LM training complete.")
